<script lang='ts'>
    ...
    const targetQuestion = getTargetQuestion(data.hash)
    ...
    const handleSubmit = async (e: SubmitEvent) => {
        ...
        formData.append('personId', data.hash)
        fetch('/api/submit', {
            method: 'POST',
            body: formData
        })
...
<h1>Hello, {data.hash}</h1>

<form class="space-y-2 p-4 bg-amber-400" onsubmit={handleSubmit} action='/api/add'>
<fieldset>
    <label>选择你打分的人</label>
            ...
            <p class="grid grid-cols-3 gap-2">
                {#each question.answerOptions as answer}
                    <label class="p-2 bg-black/10 space-x-2">
                        <input name={question.id} type='radio' value={answer.score}>
